...
export default function Budget(){
    ...
    const [ CRUDbudgetsView, toggleCRUDbudgetsView ] = useState(false);
    ...
    const CRUDbudgets = (event) => {
        ...
        */var object = categories.reduce((obj, item) => Object.assign(obj, { [item.name]: item.amount }), {});
        ...
        fetch('http://localhost:8080/CRUDbudgets',
        {
            headers: {
    ...
    function setCRUDbudgetsView(){
        toggleCRUDbudgetsView(true);
    }
    function setViewBudgetView(){
        toggleCRUDbudgetsView(false);
    }

    return(
        <div style={{width: '100vw'}}>
            <div  className="form-container">
                    <form onSubmit={CRUDbudgets}>
            ...
            {/* <div style={{display: CRUDbudgets ? 'block' : 'none'}}>
                <h2>Budget created successfully!</h2>
                <Button variant="outlined" size="large" style={{color:'white'}} onClick={()=> setCRUDbudgets(false)}>Create new budget</Button>
            </div> */}
        </div>
    );
}